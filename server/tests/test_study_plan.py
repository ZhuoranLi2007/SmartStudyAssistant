...
@pytest.mark.asyncio
async def test_study_plan_fields_and_recommended_papers(client):
    ...
    created = await client.put(f"/api/students/{parent['studentProfileId']}", headers=parent_headers, json={
        "name": "计划学生", "grade": "五年级", "subject": "数学", "recent_score": 76,
    ...
    other_account = await register(client, "other")
    forbidden = await client.get("/api/study-plans", headers=headers(other_account["accessToken"]), params={
        "student_profile_id": student["id"],
    })
    assert forbidden.status_code == 403

    student_status = await client.put(f"/api/study-plans/{task['id']}/status", headers=parent_headers, json={
        "status": "已完成",
    })
...
async def test_study_plan_family_isolation(client):
    ...
    created = await client.put(f"/api/students/{first_parent['studentProfileId']}", headers=headers(first_parent["accessToken"]), json={
